...
更新日: 2026-09-02
フェーズ: β版中心体験・内部レビュー反映完成 — `PLAN.md` B1〜B10を達成

## 現在地

記入済み詳細設計を正本に、αの画面へ機能追加する方式をやめ、講習会・開催・ロードマップのデータ境界、API、Vue UI、CSSをβとして再設計した。2026-09-02の明示依頼により基準版を指定GitHubリポジトリの`experimental/one-monthon-beta/`へ隔離し、PR #4を作成した。今回、本人指定のtraQチャンネルを認証済みChromeで読み取り専用参照し、レビュー意見を一般化してホーム検索と運営機能へ反映した。投稿、リアクション、設定変更、Vault/Daily編集、デプロイは行っていない。今回のローカル変更はcommit/pushしていない。新しい依存も追加していない。
...
- 運営向けページ: `/admin`へ案内付き講習会作成、通常フォーム、ロードマップ管理を集約。PC左ナビ/モバイル下部ナビの運営導線は同ページを開く。
