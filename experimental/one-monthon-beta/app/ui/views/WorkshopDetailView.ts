...
export default defineComponent({
  ...
  setup(props) {
    ...
    const managePreview = new URL(location.href).searchParams.get('manage') === '1';
    ...
    const load = async () => { loading.value = true; error.value = ''; try { workshop.value = await fetchWorkshop(props.workshopId, managePreview); document.title = `${workshop.value.title} | 1-Monthon β`; if (new URL(location.href).searchParams.get('saved') === '1') notice.value = '開催データを保存しました。'; await nextTick(); setRoundFromHash(); } catch (caught) { error.value = caught instanceof ApiClientError ? caught.message : '講習会を読み込めませんでした。'; } finally { loading.value = false; } };
    const toggleCompletion = async () => { if (!workshop.value || busy.value) return; const wasCompleted = workshop.value.completed; busy.value = true; error.value = ''; notice.value = ''; try { if (wasCompleted) await uncompleteWorkshop(workshop.value.id); else await completeWorkshop(workshop.value.id); workshop.value = await fetchWorkshop(props.workshopId, managePreview); notice.value = wasCompleted ? '完了記録を取り消しました。' : 'この講習会の完了を記録しました。'; } catch (caught) { error.value = caught instanceof ApiClientError ? caught.message : '完了記録を更新できませんでした。'; } finally { busy.value = false; } };
    const openMaterial = (url: string | null) => { if (url) window.open(url, '_blank', 'noopener,noreferrer'); };
    ...
    return { workshop, loading, busy, error, notice, managePreview, groups, simple, visibleGroup, primaryOccurrence, selectedRound, selectRound, toggleCompletion, openMaterial, load, formatDate, kindLabel };
  },
  template: `
      ...
      <template v-else-if="workshop">
        ...
        <div v-if="visibleGroup && primaryOccurrence" class="workshop-detail-layout">
          <div :id="String(visibleGroup.number)" class="workshop-detail-main" tabindex="-1">
            <section class="detail-section" aria-labelledby="outcome-title"><h2 id="outcome-title">この回で学べること</h2><template v-if="simple"><p class="lead-copy">{{ primaryOccurrence.description }}</p></template><div v-else class="occurrence-stack"><article v-for="occurrence in visibleGroup.occurrences" :key="occurrence.id" class="occurrence-detail"><header class="section-heading"><div><h3>{{ occurrence.title || '第' + occurrence.sequenceNumber + '回' }}</h3><p>{{ occurrence.team }} · {{ occurrence.year }}年度</p></div><span v-if="kindLabel(occurrence.kind)" class="kind-label">{{ kindLabel(occurrence.kind) }}</span></header><p>{{ occurrence.description }}</p><div v-if="occurrence.materialUrl" class="section-heading"><span></span><BasiqButton type="button" tone="neutral" variant="outline" @click="openMaterial(occurrence.materialUrl)"><AppIcon name="book" :size="18" />教材はこちら</BasiqButton></div></article></div></section>
            ...
            <section v-if="simple" class="detail-section" aria-labelledby="materials-title"><h2 id="materials-title">教材</h2><ul v-if="primaryOccurrence.materialUrl" class="material-list"><li><a :href="primaryOccurrence.materialUrl" target="_blank" rel="noreferrer"><span class="resource-mark"><AppIcon name="book" :size="19" /></span><span><strong>{{ primaryOccurrence.materialLabel }}</strong></span><span>開く</span><AppIcon name="chevron" :size="17" /></a></li></ul><div v-else class="empty-inline">教材は登録されていません。</div></section>
