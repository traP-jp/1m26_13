...
export default defineComponent({
  ...
  setup(props) {
    ...
    const primaryMaterial = computed(() => visibleGroup.value?.occurrences.find((item) => item.materialUrl) ?? null);
    ...
    const kindLabel = (kind: WorkshopOccurrence['kind']) => kind === 'rebroadcast' ? '再放送' : kind === 'digest' ? '総集編' : '通常開催';
    ...
    return { workshop, loading, busy, error, notice, groups, simple, visibleGroup, primaryOccurrence, primaryMaterial, selectedRound, selectRound, toggleCompletion, load, formatDate, kindLabel };
  },
  template: `
