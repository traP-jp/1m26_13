    ...
    <main class="page editor-page" tabindex="-1">
      ...
      <template v-else>
        ...
        <form class="editor-form" @submit.prevent="submit">
          <BasiqCard v-show="!isWizard || step === 1"><template #header><div class="form-section-title"><h2>講習会</h2><p>講習会の名前と概要を書いてください。シリーズの講習会の一部である場合は、第何回の講習会かも書いてください。</p></div></template>
            <div class="form-grid"><label class="field field-wide"><span>講習会名 <em>必須</em></span><input v-model="form.title" required maxlength="100" placeholder="例: なろう講習会第六回" :aria-invalid="Boolean(fieldErrors.title)" /></label><label class="field field-wide"><span>概要 <em>必須</em></span><textarea v-model="form.summary" required maxlength="240" rows="3" :aria-invalid="Boolean(fieldErrors.summary)"></textarea></label></div>
          </BasiqCard>
