    ...
    <main class="page discovery-page" tabindex="-1">
      ...
      <section v-else class="roadmap-catalog">
        ...
        <div v-else class="catalog-layout">
          <div class="catalog-list-wrap"><div class="results-heading"><div><h2>ロードマップ</h2><p>目的に沿って並べた講習会</p></div><strong>{{ data.roadmaps.length }}件</strong></div><ul class="roadmap-catalog-list"><li v-for="roadmap in data.roadmaps" :key="roadmap.id"><button type="button" :aria-pressed="selectedRoadmap?.id === roadmap.id" @click="selectedRoadmapId = roadmap.id"><span><strong>{{ roadmap.title }}</strong><small>{{ roadmap.summary }}</small><em>対象: {{ roadmap.audience }}</em></span><span class="catalog-progress">{{ roadmap.completedCount }}/{{ roadmap.workshopCount }}<small>完了</small></span><AppIcon name="chevron" :size="18" /></button></li></ul></div>
          <aside v-if="selectedRoadmap" class="catalog-preview" aria-label="選択したロードマップ"><BasiqCard><div class="catalog-preview-body"><span class="eyebrow">ロードマップ</span><h2>{{ selectedRoadmap.title }}</h2><p>{{ selectedRoadmap.summary }}</p><dl><div><dt>対象</dt><dd>{{ selectedRoadmap.audience }}</dd></div><div><dt>講習会</dt><dd>{{ selectedRoadmap.workshopCount }}件</dd></div><div><dt>現在の進捗</dt><dd>{{ selectedRoadmap.completedCount }}/{{ selectedRoadmap.workshopCount }} 完了</dd></div></dl><a class="primary-link" :href="'/roadmaps/' + selectedRoadmap.id" data-route>ロードマップを見る<AppIcon name="chevron" :size="18" /></a></div></BasiqCard></aside>
        </div>
      </section>
    </main>`,
});
