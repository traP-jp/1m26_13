...
function validateOccurrence(value: unknown, index: number, fields: Record<string, string>): OccurrenceInput {
  ...
  return { id: Number.isInteger(item.id) && Number(item.id) > 0 ? Number(item.id) : undefined, sequenceNumber, kind, copiedFromOccurrenceId: Number.isInteger(item.copiedFromOccurrenceId) ? Number(item.copiedFromOccurrenceId) : null, title: optionalText(item.title, 100), description: requiredText(item.description, `${key}.description`, 'この回で学べること', 3000, fields), team: requiredText(item.team, `${key}.team`, '班', 120, fields), year, scheduledAt, location: optionalText(item.location, 120) ?? '', instructor: optionalText(item.instructor, 120) ?? '', audience: requiredText(item.audience, `${key}.audience`, '対象者', 500, fields), prerequisites: optionalText(item.prerequisites, 500) ?? '', materialUrl, materialLabel: optionalText(item.materialLabel, 80) ?? '教材を開く', status };
}
