...
test('β中心導線を隔離D1で登録・公開・発見・複製・完了・ロードマップまで一巡する', { timeout: 120_000 }, async () => {
  ...
  try {
    ...
    const initial = await json(server.baseUrl, '/api/workshops'); assert.equal(initial.response.status, 200); assert.equal(initial.body.workshops.length, 10); assert.equal(initial.body.roadmaps.length, 1); assert.ok(initial.body.workshops.every((item) => item.published)); assert.ok(initial.body.workshops.some((item) => item.teams.includes('グラフィックス班'))); assert.ok(initial.body.workshops.some((item) => item.teams.includes('ゲーム班')));
    const initialHome = await json(server.baseUrl, '/api/workshops?includeDrafts=1'); assert.equal(initialHome.body.workshops.length, 11); const draftSummary = initialHome.body.workshops.find((item) => item.id === 4); assert.equal(draftSummary.published, false); assert.deepEqual(draftSummary.teams, ['SysAd班']); assert.deepEqual(draftSummary.years, [2027]);
    ...
    const homeAfterCreate = await json(server.baseUrl, '/api/workshops?includeDrafts=1'); const createdSummary = homeAfterCreate.body.workshops.find((item) => item.id === id); assert.equal(createdSummary.published, false); assert.deepEqual(createdSummary.teams, []); assert.deepEqual(createdSummary.years, []); assert.equal(createdSummary.occurrenceCount, 0);
    ...
    const occurrenceRemoved = await json(server.baseUrl, `/api/workshops/${id}`, { method: 'PUT', body: JSON.stringify({ ...updatedInput, occurrences: updatedInput.occurrences.filter((item) => item.id !== copy.id).map((item) => ({ ...item, sequenceNumber: 1 })) }) }); assert.equal(occurrenceRemoved.response.status, 200); assert.deepEqual(occurrenceRemoved.body.workshop.occurrences.map((item) => item.id), [occurrenceId]); assert.deepEqual(occurrenceRemoved.body.workshop.occurrences.map((item) => item.sequenceNumber), [1]);
    ...
    const roadmapPayload = { title: 'API管理ロードマップ', summary: '作成・編集・削除を確認します。', audience: '内部試用者', published: false, stages: [{ items: [{ workshopId: 1, note: '先に受ける' }] }, { items: [{ workshopId: 2, note: '' }] }] };
    ...
    assert.equal(createdRoadmap.body.roadmap.published, false); assert.equal('title' in createdRoadmap.body.roadmap.stages[0], false); assert.equal('description' in createdRoadmap.body.roadmap.stages[0], false);
    const publishedRoadmap = await json(server.baseUrl, `/api/roadmaps/${createdRoadmapId}`, { method: 'PUT', body: JSON.stringify({ ...roadmapPayload, title: '更新済みAPI管理ロードマップ', published: true }) }); assert.equal(publishedRoadmap.response.status, 200); assert.equal(publishedRoadmap.body.roadmap.published, true);
