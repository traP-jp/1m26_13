...
更新日: 2026-09-04
フェーズ: 本番再設計 — 統合コンテキストのGrilling監査、P0仕様の確定が次のチェックポイント

## 現在地

本番はβ版を改修して作らず、フロムスクラッチで構築する方針に変更した。β版は動作する検証資料として固定し、人間が加えた差分のうち採用するものだけを、問題・挙動・根拠・適用条件を持つ意思決定として本番コンテキストへ戻す。統合コンテキストと`badge alpha`の確定後、`SPEC_GAP_AUDIT.md`で境界・状態遷移・例外を監査した。P2へ入る前に、Flow、完了、公開ライフサイクル、ロードマップ、権限、削除・参照切れのP0仕様を確定する。

`BETA_CHANGE_AUDIT.md`でβ差分を採用・不採用へ二分した。学年度の日時導出、再放送元の単一化、ロードマップ段階情報の削除、最後のSession削除禁止、完了記録のcascade削除など、現行仕様から情報や能力を失う変更は回帰として不採用にした。採用したのは技術・配置から独立した挙動とアクセシビリティ原則であり、βコードや画面外観は本番へ継承しない。

P1を完了した。採用した機能・データ判断を`PRODUCTION_CONTEXT.md`へ統合し、画面に関するβの判断・観察・スクリーンショット索引は`BETA_SCREEN_DESIGN_REFERENCE.md`へ分離した。画面資料は本番仕様ではなく、P2で新しい画面案と比較する入力として使う。次のチェックポイントは本番画面設計。

記入済み詳細設計を正本に、αの画面へ機能追加する方式をやめ、講習会・開催・ロードマップのデータ境界、API、Vue UI、CSSをβとして再設計した。2026-09-02の明示依頼により基準版を指定GitHubリポジトリの`experimental/one-monthon-beta/`へ隔離し、PR #4を作成した。本人指定のtraQチャンネルをCLIで参照し、レビュー意見を一般化して反映している。Inter/M PLUS 1pの5.3.0に続き、beta.3のvendor更新依頼と本タスクでのパッケージ単位例外の明示依頼に対応した。`basiq-ui`だけを公開後3日制限から除外し、対応版npmで正規配布物を取得・検証済み。traQへの投稿・編集、許可済みeyes以外のリアクション、外部設定変更、Vault/Daily編集、デプロイは行っていない。
...
- 仕様Grilling監査: `SPEC_GAP_AUDIT.md`へP0 6群、P1 7群、文書間の不一致3件を整理した。画面設計だけでは解決できない項目と、P2で決めてよい項目を分離した。
- バッジalpha: `references/badge-alpha.html`を原本と同一ハッシュで固定し、`BADGE_ALPHA_SPEC.md`へ講習会バッジの意味、安定IDによる決定的生成、視覚規則、表示情報、モーション低減、アクセシビリティ、画面設計へ残す事項を整理した。
- データ構造: 添付された`lecture_session_spec_v2.md`を`LECTURE_SESSION_FLOW_SPEC.md`として取り込み、Lectureを講習会全体、Sessionを1日で完結する各開催とする2階層を正本にした。既存D1は非破壊移行のため物理テーブル名を当面維持する。
- フロー: 従来のウィザードをフローへ改称。`lecture_pre`（講習会の事前）、`session_main`（各開催のメイン）、`lecture_post`（講習会の事後）を追加し、属性ごとにLectureまたはSessionを1件だけ参照するDB/API制約を実装した。
- フロー管理: 運営向けページでフローの作成、一覧、編集、削除ができる。APIは`GET/POST /api/flows`と`PUT/DELETE /api/flows/{id}`。
- 検証: `npm run check:static`成功（ドメイン14件、traQ確認8件）。隔離D1の`npm run test:api`で3属性の対象制約とFlow CRUD、既存中心導線を確認。`npm run build`は全5段階とFlow APIを含む全ルートで成功。
- パンくず: 画面内の区切り文字を装飾アイコンから`/`へ統一し、読み上げ対象から除外した。
...
- UI: Vue 3を継続し、`BasiqThemeProvider`、`Button`、`Card`、`ChoiceGroup`、`ChoiceGroupItem`、`ToggleButton`を実コンポーネントとして使用。1,034行だった単一CSSは、共通import元と`base/layout/forms/pages`へ分割。
- フォント: Inter/M PLUS 1pをFontsource 5.3.0から自己配信。既存のbasiQ-uiフォントトークンをそのまま使い、本文・カード・フォーム・ボタンへ適用する。追加CSSは配布元のfont-face importのみ。ライセンスは`/font-licenses.txt`で参照できる。
...
- プロフィール: バッジ、完了した講習会、ロードマップを下線型の3タブで表示。beta.0時点でTabsがなかったためnativeのARIA tabをbasiQ tokenで最小実装し、選択/対応関係と左右矢印・Home・Endのキーボード移動を備える。beta.3で追加されたTabsへの移行は今回の依存更新とは分離する。バッジ詳細は「受講完了日」を表示し、講習会詳細への導線をカード中央に置く。
...
- 2026-09-03 パッケージ限定例外・basiQ-ui beta.3導入後:
  - `npm exec --yes --package=npm@11.19.1 -- npm config get min-release-age min-release-age-exclude`: `3`と`basiq-ui`を確認。既定npm 11.16は未対応のため、一時実行する11.19.1だけで取得・導入する。グローバル設定は未変更。
  - 同じnpmで`npm pack basiq-ui@0.1.0-beta.3 --ignore-scripts --json`と`npm install --ignore-scripts`成功。MIT、integrity一致、既存6部品のAPI維持、削除トークン0件を監査し、変更パッケージはbasiq-uiの1件だけ。
  - `npm exec --yes --package=npm@11.19.1 -- npm run check`: TypeScript、ESLint、ドメイン12/12、traQ CLI 8/8、隔離D1 API統合1/1、Vinext build全5段階が成功。fail/skip/todo 0。
  - Chrome PC 1414×1000: 27件3列カード、Git絞り込み1件、単発詳細/教材と回選択なし、完了保存→プロフィールのバッジ1件/完了1件、タブ右矢印で選択とフォーカスが連動。`#3`直アクセス→実ChoiceGroupの第2回Enter操作で`#2`と本文フォーカスへ移動。
  - Chrome 390×844: 通常/再放送の同一回表示、27件一覧、ロードマップ実ToggleButtonのクリック/Space操作と保存成功、1/2進捗、共有開閉、必須入力の2px outline、空検索→条件クリア、存在しない編集画面の失敗/再試行を確認。PC一覧・モバイル再放送・フォームのスクリーンショットを目視し、確認画面の横あふれ0、console warning/error 0件。
  - 標準フォントは`loaded`、Inter/M PLUS 1pの文字チェック成功。アプリのUIソース・CSSは変更不要。Chrome操作は`/private/tmp/one-monthon-beta3-qa.oZCcki`の隔離D1だけを使用し、利用者D1は変更しない。
  - `npm exec --yes --package=npm@11.19.1 -- npm audit --omit=dev`: 0件。最初のsandbox内実行はDNS制限で失敗し、承認済みネットワークで再実行して成功。旧npm起動時の設定警告はREADMEに説明済みで、対応版npm本体やブラウザの警告ではない。

- 2026-09-03 標準フォント導入後:
  - `npm install --ignore-scripts`: 指定フォント2パッケージだけを追加、lockfileのその他の依存は変更なし。両方OFL-1.1、推移依存なし。
  - `npm run check`: 成功。TypeScript、ESLint、ドメイン12/12、traQ CLI 8/8、隔離D1 API統合1/1、Vinext build全5段階。fail/skip/todo 0。
  - ビルドCSSのフォント参照1195件を検査し、外部URL 0、参照ファイル欠落0、配布先の`font-licenses.txt`あり。
  - Chrome PC（1414px）/390×844: フォント定義635件、`document.fonts.status = loaded`、Inter英字/M PLUS 1p日本語の`document.fonts.check`成功。本文、見出し、実basiQ-uiボタン、input/textareaが同じ標準フォント指定を継承することを確認。
  - ホーム検索→単発詳細/教材→完了→プロフィールのバッジ/履歴→ロードマップ1/2進捗、共有開閉、`#3`直リンクと第2回切替、同じ回の通常/再放送を確認。PC/390pxのスクリーンショットを目視し、確認画面の横あふれ0。
  - 390pxの空必須フォームでinputへフォーカス、青2px outlineを確認。新規講習会→開催入力/公開→basiQ-ui関連講習会トグル→保存後詳細→再読込の保持を実操作。空検索の条件クリア、存在しない編集画面の失敗/再試行、編集入力0件も確認。console warning/error 0件。
  - Chrome確認は`/private/tmp/one-monthon-b36.34Te8t`の隔離D1のみを使用。通常の利用者D1とVault/Dailyは変更していない。
  - `npm audit --omit=dev`: 0件。全依存監査では既存の開発専用fast-uri high 1件、drizzle-kit系moderate 4件。追加フォントに監査警告なし。
  - `npm pack basiq-ui@0.1.0-beta.3 --ignore-scripts --json`: 環境の最小公開経過時間制限でETARGET。制限回避はせず、next本体は未更新として下記へ記録。
...
- 2026-09-02 追加内部レビュー反映後:
  ...
  - Chrome PC: `/admin`の講習会作成操作が1ボタンだけで、通常フォームの選択肢がないことを確認。ボタンから`/admin/workshops/new`の3段階フローへ遷移した。
...
- 開発専用`drizzle-kit`系moderate 4件と`fast-uri` high 1件の既存監査警告。追加フォント2パッケージは無関係で、本番依存監査は0件。
- beta.3追加のInput/FormField/Tabs等への既存native UIの置換は未実施。今回使う6部品の互換性を検証し、画面改修とは分離した。設定対応のためnpm 11.19以上が必要（READMEに一時実行手順あり）。
...
- なし。beta.3取得の旧ブロッカーは本人指定の限定例外と対応版npmにより解消した。
