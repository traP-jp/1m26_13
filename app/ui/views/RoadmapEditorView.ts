...
const blankStage = (): RoadmapInputStage => ({ title: '', description: '', items: [] });
...
export default defineComponent({
  ...
  emits: ['navigate'],
  setup(props, { emit }) {
    const form = ref<RoadmapInput>({ title: '', summary: '', audience: '', status: 'draft', stages: [blankStage()] });
    ...
    const load = async () => {
      ...
      try {
        ...
        if (isEdit.value) {
          ...
          form.value = { title: roadmap.title, summary: roadmap.summary, audience: roadmap.audience, status: roadmap.status, stages: roadmap.stages.map((stage) => ({ title: stage.title, description: stage.description, items: stage.items.map((item) => ({ ...item })) })) };
        } else form.value = { title: '', summary: '', audience: '', status: 'draft', stages: [blankStage()] };
  ...
  template: `
    <main class="page roadmap-editor-page" tabindex="-1">
      ...
      <header class="page-heading"><div><p class="eyebrow">ロードマップ管理</p><h1>{{ isEdit ? 'ロードマップを編集' : 'ロードマップを作成' }}</h1><p>閲覧ページに表示する段階と講習会の順番を設定します。</p></div></header>
      ...
      <form v-else class="roadmap-editor-form" @submit.prevent="submit">
        ...
        <BasiqCard title="基本情報">
          <div class="form-grid two-columns">
            ...
            <label class="field"><span>公開状態</span><select v-model="form.status"><option value="draft">下書き</option><option value="published">公開</option></select></label>
          </div>
        </BasiqCard>

        <section class="roadmap-stage-editor" aria-labelledby="stage-editor-title">
          <div class="section-heading"><div><h2 id="stage-editor-title">段階と講習会</h2><p>上から順に閲覧ページへ表示されます。</p></div><BasiqButton type="button" tone="neutral" variant="outline" @click="addStage">段階を追加</BasiqButton></div>
          ...
          <ol class="roadmap-stage-editor-list">
            <li v-for="(stage, stageIndex) in form.stages" :key="stageIndex">
              <BasiqCard>
                ...
                <div class="form-grid">
                  <label class="field"><span>段階名 <em>必須</em></span><input v-model="stage.title" required maxlength="100" /></label>
                  <label class="field"><span>説明</span><textarea v-model="stage.description" maxlength="500" rows="2"></textarea></label>
