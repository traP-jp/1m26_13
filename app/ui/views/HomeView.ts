...
export default defineComponent({
  ...
  setup() {
    ...
    const load = async () => {
      loading.value = true;
      error.value = '';
      try { data.value = await fetchDiscovery('', '', '', true); }
  ...
  template: `
    <main class="page home-page" tabindex="-1">
      <header class="page-heading compact-heading">
        <div><h1>ホーム</h1><p>講習会と教材を見つけます。</p></div>
      </header>
      ...
      <section class="home-section" aria-labelledby="workshops-title">
        <div class="section-heading"><div><h2 id="workshops-title">講習会</h2><p>作成中を含む講習会を表示しています。</p></div><span v-if="!loading && !error">{{ data.workshops.length }}件</span></div>
        ...
        <div v-else-if="!data.workshops.length" class="empty-inline">講習会はありません。</div>
