...
const compositionTypes = ['SEGMENTED RING', 'SOLID CREST', 'INTERLOCKING BLOCKS', 'RADIAL CHEVRONS', 'MODULAR GRID', 'HEAVY POLYGON'] as const;

function createArcSlice(cx: number, cy: number, outerRadius: number, innerRadius: number, startAngle: number, endAngle: number) {
...
function createPolygonPoints(cx: number, cy: number, radius: number, sides: number, startAngle = 0) {
  const points: string[] = [];
...
function generateIdentifier(rng: SeededRng) {
  const first = ['SOLID', 'HEAVY', 'BLOCK', 'APEX', 'PRISM', 'KINETIC', 'MODULE', 'STRUCT', 'NEXUS', 'CORE'] as const;
  const second = ['FORGE', 'CREST', 'MARK', 'GUARD', 'INDEX', 'UNIT', 'SHIELD', 'ALPHA', 'PRIME'] as const;
  ...
  switch (params.coreType) {
    ...
    case 1: {
      ...
      for (let index = 0; index < pointCount * 2; index += 1) {
        ...
        points.push(`${(cx + pointRadius * Math.cos(angle)).toFixed(2)},${(cy + pointRadius * Math.sin(angle)).toFixed(2)}`);
      }
