...
test('β中心導線を隔離D1で登録・公開・発見・複製・完了・ロードマップまで一巡する', { timeout: 120_000 }, async () => {
  ...
  try {
    ...
    const occurrenceRemoved = await json(server.baseUrl, `/api/workshops/${id}`, { method: 'PUT', body: JSON.stringify({ ...updatedInput, occurrences: updatedInput.occurrences.filter((item) => item.id !== copy.id) }) }); assert.equal(occurrenceRemoved.response.status, 200); assert.deepEqual(occurrenceRemoved.body.workshop.occurrences.map((item) => item.id), [occurrenceId]);
